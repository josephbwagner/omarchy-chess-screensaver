...
def calculate_layout(lines, cols):
    ...
    # Board panel: at least 34 cols wide (2 rank label + 8×4 squares), target 2/3 of interior
    board_panel_width = max(34, math.floor(interior_cols * 2 / 3))

    # Ensure the info panel gets at least 20 chars; clamp board panel if needed
    if board_panel_width > interior_cols - 21:
        board_panel_width = max(34, interior_cols - 21)
    ...
    # Board content dimensions: 34 wide (2 rank label + 8×4 squares), 9 tall (8 ranks + file row)
    board_content_w = 34
    board_content_h = 9
